...
#define KEYCODE_SPACE 0x20 // 空格急停
// IO Keys
#define KEYCODE_1 0x31 // Toggle Beam
#define KEYCODE_2 0x32 // Toggle Turn Left
#define KEYCODE_3 0x33 // Toggle Turn Right
...
{
  ros::init(argc, argv, "yhs_keyboard_node");
  ros::NodeHandle nh;

  ros::Publisher ctrl_pub = nh.advertise<yhs_can_msgs::ctrl_cmd>("ctrl_cmd", 10);
  ros::Publisher io_pub = nh.advertise<yhs_can_msgs::io_cmd>("io_cmd", 10);
  ...
  ros::Duration timeout_duration(0.2); // 200ms 超时判定
  ...
  while(ros::ok())
  {
    ...
    if (is_moving && (ros::Time::now() - last_key_time > timeout_duration))
    {
      target_speed = 0.0;
      target_turn = 0.0;
      dirty_motion = true; 
      is_moving = false;
      // ROS_INFO("Auto Stop Triggered!"); // 调试用
    }
    ...
    if (dirty_io)
    {
       ...
       ROS_INFO("IO Updated: Beam=%d, Turn=%d", io_msg.io_cmd_lower_beam_headlamp, io_msg.io_cmd_turn_lamp);
    }
    ...
    if (dirty_motion)
    {
      ...
      // 档位逻辑
      if (target_speed > 0.001) cmd_msg.ctrl_cmd_gear = 0x04; // D档
      else if (target_speed < -0.001) cmd_msg.ctrl_cmd_gear = 0x02; // R档
      else cmd_msg.ctrl_cmd_gear = 0x03; // N档 (停车)
